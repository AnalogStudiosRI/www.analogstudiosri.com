...
const { handler } = await import("#pages/api/posts.ts");
...
describe("Posts API", () => {
  ...
  it("should return all posts", async () => {
    const response = await handler(new Request("http://localhost:8080/api/posts"));
  ...
  it("should return a post by id", async () => {
    ...
    const response = await handler(new Request(`http://localhost:8080/api/posts?id=${id}`));
